...
// Danh sách cột đúng theo DB của bạn
const TRAINER_ATTRIBUTES = [
  'id',
  'userId',
  'gymId', // ✅ thêm để trả gymId ra API (không đổi logic, chỉ thêm field)
  'specialization',
  'certification',
  'experienceYears',
  'hourlyRate',
...
exports.createTrainer = async (req, res) => {
  try {
    ...
    // ✅ BẮT BUỘC gymId khi tạo PT (để PT tạo package auto theo gym về sau)
    if (!payload.gymId) {
      return res.status(400).json({
    ...
    // Trả về record chuẩn attributes
    const created = await TrainerModel.findByPk(newTrainer.id, {
      attributes: TRAINER_ATTRIBUTES,
      raw: true,
    });
